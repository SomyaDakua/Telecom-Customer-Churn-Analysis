...
SELECT @Total_cust AS Total_Customers,@Churned_cust AS Churned_cutomers,
CONCAT(ROUND(1.00*(SELECT @Churned_cust)/(SELECT @Total_cust)*100,2),'%') AS Average_Churn_Rate;
...
SELECT @Churned_cust AS Total_Churned_Customers,@Male_Churned AS Male,@Female_Churned AS Female;
...
/**Tenure Analysis**/
#Tenure analysis for customers who churned
...
#Tenure analysis for customers who joined
SELECT COUNT(Customer) AS Joined_Customers,
ROUND(Tenure_in_Months/12) AS Tenure_in_Years
FROM telecom
...
SELECT COUNT(Customer) AS Customers,Payment_Method FROM telecom
WHERE Customer_Status='Churned'
...
WHERE Customer_Status='Churned' AND Paperless_Billing='No') AS Churned_Cust_not_using_pb,
 ...
 SELECT Customer,Paperless_Billing FROM telecom
 Where Customer_Status='Churned';
